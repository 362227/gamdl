...
class AppleMusicBaseInterface:
    ...
    async def get_playready_decryption_key(
        self,
        pssh: str,
        track_id: str,
    ) -> DecryptionKey:
        """Get a PlayReady key through the combined wrapper service."""
        if self.wrapper_api is None:
            raise ValueError("wrapper_api is required for PlayReady decrypt")

        helper = os.environ.get("GAMDL_PLAYREADY_HELPER", "gamdl-playready")
        ...
        if result.returncode != 0:
            raise RuntimeError(result.stderr.strip() or "PlayReady helper failed")

        key = result.stdout.strip()
        if len(key) != 32:
            raise RuntimeError("PlayReady helper returned an invalid content key")
        try:
            bytes.fromhex(key)
        except ValueError as exc:
            raise RuntimeError("PlayReady helper returned a non-hex content key") from exc

        return DecryptionKey(key=key, kid=None)
