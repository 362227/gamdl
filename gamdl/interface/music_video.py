...
class AppleMusicMusicVideoInterface:
    ...
    async def _get_stream_info(
        ...
        # Music-video audio and video can carry different PlayReady KIDs.
        # Do not inherit the generic single-content-key default here; obtain
        # an audio license separately when the audio track has its own PSSH.
        stream_info_video.use_single_content_key = False
        stream_info_audio.use_single_content_key = False
    ...
    async def _get_stream_info_video(
        ...
        print(f"m3u8: {stream_info.stream_url}")  # 加在这里
    ...
    async def get_decryption_key(
        ...
        if (
            ...
            and stream_info.audio_track.playready_pssh
        ):
            get_key = self.base.get_playready_decryption_key
            video_pssh = stream_info.video_track.playready_pssh
            audio_pssh = stream_info.audio_track.playready_pssh
        elif (
            stream_info.video_track.widevine_pssh
            and stream_info.audio_track.widevine_pssh
        ):
            get_key = self.base.get_decryption_key
            video_pssh = stream_info.video_track.widevine_pssh
            audio_pssh = stream_info.audio_track.widevine_pssh
        else:
            raise GamdlInterfaceDecryptionNotAvailableError(stream_info.media_id)

        decryption_key_video = await get_key(video_pssh, stream_info.media_id)
        if stream_info.video_track.use_single_content_key:
    ...
    async def get_media(
        ...
        if (
            not (
                media.stream_info.video_track.widevine_pssh
                or media.stream_info.video_track.playready_pssh
            )
            or not (
                media.stream_info.audio_track.widevine_pssh
                or media.stream_info.audio_track.playready_pssh
            )
        ):
